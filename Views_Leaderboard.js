...
app.views.leaderboard = function(spieltagId, activeTab)
{
    ...
    if (spieltagId)
    {
        spieltag = (app.state.spieltage || []).find(
            function(st) 
            { 
    ...
    if (!spieltag && app.state.spieltage && app.state.spieltage.length > 0)
    {
        ...
        if (!spieltag)
        {
            const sortedRounds = [...app.state.spieltage].sort(
                function(a, b) 
                { 
    ...
    const platz = kurs ? (app.state.golfplaetze || []).find(
        function(p) 
        { 
    ...
    if (spieltag)
    {
        ...
        {
            teilnehmerIds = spieltag.teilnehmerCsv.map(
                function(id) 
                { 
        ...
        {
            teilnehmerIds = spieltag.teilnehmerCsv.split(',').map(
                function(id) 
                { 
                    return String(id).trim(); 
                }
        ...
        {
            const tagesFlights = app.state.flights.filter(
                function(f) 
                { 
                    return String(f.spieltagId || "").trim() === String(spieltag.id || "").trim(); 
                }
            );
            tagesFlights.forEach(
                function(f)
                {
                    const ids = String(f.spielerIdsCsv || "").split(',').map(
                        function(id) 
                        { 
                            return String(id).trim(); 
                        }
                    ...
                    ids.forEach(
                        function(id) 
                        { 
                            if (id && !teilnehmerIds.includes(id)) 
                            {
    ...
    // RENDERING LISTE
    let rowsListHtml = leaderboardData.map(
        function(row, index)
        {
    ...
    let tableHeaderHoles = anzuzeigendeBahnen.map(
        function(b) 
        { 
            return `<th class="p-1 text-center border-zinc-200 font-bold text-zinc-700 min-w-[32px] text-xs">${b.nr}</th>`; 
        }
    ).join('');
    let tableHeaderPars = anzuzeigendeBahnen.map(
        function(b) 
        { 
            return `<td class="p-1 text-center border-zinc-100 text-zinc-400 text-[10px] font-medium bg-zinc-50">${b.par}</td>`; 
        }
    ).join('');

    let tableRowsHtml = leaderboardData.map(
        function(row)
        {
            let scoreCells = anzuzeigendeBahnen.map(
                function(b)
                {
...
{
    const spieltag = (app.state.spieltage || []).find(
        function(s)
        {
    ...
    const kursBahnen = (app.state.bahnen || [])
        .filter(
            function(b)
            {
                return String(b.kursId).trim() === String(spieltag.kursId).trim();
            }
        )
        .sort(
            function(a, b)
    ...
    const dbScores = (app.state.scoreCards || []).filter(
        function(sc)
        {
            return String(sc.spieltagId).trim() === String(spieltagId).trim() && String(sc.spielerId).trim() === String(spielerId).trim();
        }
    );
        ...
        function(bahn)
        {
            ...
            if (match && (rawStrokes > 0 || isStrich)) 
            {
                ...
                if (isStrich) 
                {
                    ...
                    const strokeDisplay = rawStrokes > 0 ? `${rawStrokes} <span class="text-zinc-400 font-normal text-[11px]">Ø</span>` : 'Ø';
                    ...
                    return `
                        <tr class="border-b border-zinc-100 text-xs bg-zinc-50/50">
                            ...
                            <td class="py-2 px-1 text-center font-bold text-zinc-800">${strokeDisplay} ${puttsDisplay} ${isLady ? '🍻' : ''}</td>
